...
it('should replace regular wildcards with sql and escape sql wildcards', () => {
  const { sqlQuery: actualQuery } = queryBuilder({ searchQuery: 'lul._lul%.*lul', currentNetworkNumber: 6671 })
  const expectedQuery =
`
SELECT
\tOffice.vw_LoadNodes.NetNumber as NetNumber,
...
\tON Office.vw_LoadNodes.UserId = Ncc.[User].ID
WHERE
\t(Office.vw_LoadNodes.Name LIKE '%lul_[_]lul[%]%lul%')
`
